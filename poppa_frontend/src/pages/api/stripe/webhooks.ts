  ...
  switch (event.type) {
    case "checkout.session.completed":
      ...
      try {
        ...
        let usageLimit = 0;
        if (priceId === process.env.NEXT_PUBLIC_STRIPE_PRICE_ID_HOBBY) {
          // Example price ID
          usageLimit = 1000;
        ...
        const { error: usageError } = await supabaseClient.from("usage").upsert(
          {
            user_id: userId,
    ...
    case "invoice.payment_failed":
      ...
      try {
        ...
        if (updateError) {
          console.error(
            "❌ Error updating subscription status for invoice.payment_failed:",
            updateError
          );
