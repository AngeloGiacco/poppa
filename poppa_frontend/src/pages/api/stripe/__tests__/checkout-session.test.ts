...
describe('/api/stripe/checkout-session API Endpoint', () => {
  ...
  it('should return 400 if price_id is missing', async () => {
    ...
    expect(mockRes._getJSONData()).toEqual({ error: 'Missing price_id or user_id in request body.' });
  });
  ...
  it('should handle errors from Supabase when fetching customer ID', async () => {
    ...
    expect(consoleErrorSpy).toHaveBeenCalledWith('Error fetching existing subscription for customer ID:', dbError);
