  ...
  try {
    ...
    switch (queryType) {
      case "check_vocabulary":
        ...
        result = await checkVocabularyHistory(
          userId,
          languageCode,
      ...
      case "get_related_vocabulary":
        if (!params?.category) {
          return Response.json(
            { error: "Missing param: category" },
            { status: 400 }
          );
        }
        result = await getRelatedVocabulary(
          userId,
          languageCode,
      ...
      case "natural_language":
        ...
        result = await queryMemoryNaturalLanguage(
          userId,
          languageCode,
    ...
    return Response.json(
      { error: "Failed to execute memory query" },
      { status: 500 }
    );
  }
}
